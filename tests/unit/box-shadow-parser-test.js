...
module("Unit | BoxShadow", function(hooks) {
  ...
  function example(authoredShadow, expectedShadows) {
    test('parsing ' + authoredShadow, function(assert) {
      ...
      let shadows = BoxShadow.fromComputedStyle(getComputedStyle(target)['box-shadow']);
      ...
      expectedShadows.forEach((expectedShadow, index) => {
        ...
        assert.equal(shadow.blur, expectedShadow.blur || 0, `shadow ${index} blur`);
        assert.equal(shadow.spread, expectedShadow.spread, `shadow ${index} spread`);
        assert.equal(shadow.inset, expectedShadow.inset || false, `shadow ${index} inset`);
        ...
        assert.equal(shadow.color.a, expectedShadow.a == null ? 1 : expectedShadow.a, `shadow ${index} a`);
      });
    });
  }
  ...
  example(`none`, []);
  example(``, []);
  example(`0 0 20px black`, [{ x: 0, y: 0, blur: 20, spread: 0, r: 0, g: 0, b: 0}]);
  example(`1px 2px 3px 4px rgba(5, 6, 7, 0.5) inset`, [{ x: 1, y: 2, blur: 3, spread: 4, r: 5, g: 6, b: 7, a: 0.5, inset: true}]);
