...
import { MotionTester, time, setupAnimationTest } from 'ember-animated/test-support';
...
`;
  ...
  test("uses existing colors", async function(assert) {
    ...
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'red');
    await time.advance(499);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'purple');
    await time.advance(999);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'blue');
  });
  ...
  test("explicit source color", async function(assert) {
    ...
    target.style['background-color'] = 'white';
    sprite.measureFinalBounds();
    let motion = new AdjustColor('background-color', sprite, { from: 'yellow' });
    time.pause();
    tester.run(motion, { duration: 1000 });
    await time.advance(1);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'yellow');
    await time.advance(499);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], '#ffff7f');
    await time.advance(999);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'white');
  });
  ...
  test("handles alpha premultiplication", async function(assert) {
    ...
    tester.run(motion, { duration: 1000 });
    await time.advance(1);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], '#0000ff');
    await time.advance(499);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], 'rgba(84, 0, 170, 0.75)');
    await time.advance(999);
    assert.approxEqualColors(getComputedStyle(target)['background-color'], '#ff00007f');
  });
