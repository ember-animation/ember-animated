...
module('Integration | Sprites Example', function(hooks){
  ...
  test('it renders', async function(assert) {
    ...
    {{sprites-example}}
    `);
    assert.equal(this.element.querySelectorAll('.top-bar > button').length, 2, "found two buttons");
  });
  ...
  test('adding an email', async function(assert) {
    ...
    assert.ok(bounds(emails[0]).left > bounds(emails[1]).left, "first email to the right of the others at start of animation");
  });
  ...
  test('transitions get logged to screen', async function(assert) {
    ...
    assert.equal(rows.length, 3, "should have one log entry (first two rows are headers)");
