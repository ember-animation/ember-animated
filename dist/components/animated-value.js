...
import AnimatedEach from './animated-each.js';
...
class AnimatedValueComponent extends Component {
  ...
  */
  ...
  static {
    setComponentTemplate(precompileTemplate("\n    {{#AnimatedEach this.items key=this.key rules=this.rules use=this.use duration=this.duration group=this.group watch=this.watch initialInsertion=this.initialInsertion finalRemoval=this.finalRemoval as |item|}}\n      {{yield item}}\n    {{/AnimatedEach}}\n  ", {
      strictMode: true,
      scope: () => ({
        AnimatedEach
      })
