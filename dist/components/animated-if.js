...
import AnimatedValueComponent from './animated-value.js';
...
class AnimatedIfComponent extends Component {
  ...
  static {
    setComponentTemplate(precompileTemplate("\n    <AnimatedValue @value={{this.predicate}} @key={{this.key}} @rules={{this.rules}} @use={{this.use}} @duration={{this.duration}} @group={{this.realGroup}} @initialInsertion={{this.initialInsertion}} @finalRemoval={{this.finalRemoval}} as |currentPredicate|>\n      {{#if currentPredicate}}\n        {{yield}}\n      {{else}}\n        {{yield to=\"inverse\"}}\n      {{/if}}\n    </AnimatedValue>\n  ", {
      strictMode: true,
      scope: () => ({
        AnimatedValue: AnimatedValueComponent
      })
    }), this);
  }
}
